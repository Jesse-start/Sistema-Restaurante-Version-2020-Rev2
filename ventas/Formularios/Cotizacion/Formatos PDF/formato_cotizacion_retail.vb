...
Module formato_cotizacion_retail
    Public Sub Generar_Formato_cotizacion_retail(ByVal id_cotizacion As Integer, ByVal cancelado As Boolean)
        'Tryn
        'Intentar generar el documento.
        Dim doc As New Document(PageSize.LETTER, 30, 40, 20, 45)
        Dim filename As String = global_dir_cotizaciones + "\COTIZACION_" & Format(id_cotizacion, "0000") & ".pdf"
        Dim file As New FileStream(filename, FileMode.Create, FileAccess.Write, FileShare.ReadWrite)
        Dim writer As PdfWriter = PdfWriter.GetInstance(doc, file)
        doc.Open()
        Generar_pagina(doc, writer, id_cotizacion, -2565928)
        doc.Close()
    ...
    End Sub
    Public Sub Generar_pagina(ByVal document As Document, ByVal writer As PdfWriter, ByVal id_cotizacion As Integer, ByVal color_fondo As String)
        ...
        Dim saldo As String = ""
        Dim total_cotizacion As Decimal = 0
        Dim fecha_cotizacion As DateTime
        Dim serie_cotizacion As String = ""
        ...
        rs.Open("SELECT c.fecha,c.numero,c.total,c.id_cliente,c.id_empleado,c.mensaje,c.despedida,c.nombre_cliente,CONCAT(p.nombre,' ',p.ap_paterno,' ',p.ap_materno) AS empleado_elabora " & _
                "FROM cotizacion c " & _
                "JOIN empleado e ON e.id_empleado=c.id_empleado " & _
                "JOIN persona p ON p.id_persona=e.id_persona " & _
                "WHERE id_cotizacion='" & id_cotizacion & "'", conn)
        If rs.RecordCount > 0 Then
            id_cliente = rs.Fields("id_cliente").Value
            fecha_cotizacion = rs.Fields("fecha").Value
            vendedor = rs.Fields("empleado_elabora").Value
            total_cotizacion = rs.Fields("total").Value
        ...
        Dim A6 As New PdfPCell(New Paragraph(" COTIZACIÓN", FontFactory.GetFont("Arial", 14, Font.BOLD)))
        ...
        Dim B6 As New PdfPCell(New Paragraph(serie_cotizacion & " " & Format(id_cotizacion, "0000"), FontFactory.GetFont("Arial", 12, Font.BOLD)))
        B6.Colspan = 3
        ...
        Dim D6 As New PdfPCell(New Paragraph(fecha_cotizacion.Day, FontFactory.GetFont("Arial", 10, Font.BOLD)))
        D6.HorizontalAlignment = Element.ALIGN_CENTER
        NOTA.AddCell(D6)

        Dim D7 As New PdfPCell(New Paragraph(fecha_cotizacion.Month, FontFactory.GetFont("Arial", 10, Font.BOLD)))
        D7.HorizontalAlignment = Element.ALIGN_CENTER
        NOTA.AddCell(D7)

        Dim D8 As New PdfPCell(New Paragraph(fecha_cotizacion.Year, FontFactory.GetFont("Arial", 10, Font.BOLD)))
...
"Tel:. 14 381 44 Email:eltropicomateriales@gmail.com", FontFactory.GetFont("Arial", 9, Font.BOLD)))
        ...
        Dim J1 As New PdfPCell(New Paragraph("ELABORÓ:", FontFactory.GetFont("Arial", 8, Font.BOLDITALIC)))
        J1.Border = Nothing
        J1.MinimumHeight = 15
        J1.PaddingTop = 10
        J1.PaddingBottom = 10
        ...
        Dim J4 As New PdfPCell(New Paragraph("", FontFactory.GetFont("Arial", 8, Font.BOLDITALIC)))
        J4.Border = Nothing
        J4.PaddingTop = 10
        J4.PaddingBottom = 10
        J4.Colspan = 2
        ...
        Dim J6 As New PdfPCell(New Paragraph("", FontFactory.GetFont("Arial", 8, Font.UNDERLINE)))
        J6.Border = Nothing
        J6.PaddingTop = 10
        J6.PaddingBottom = 10
        J6.Colspan = 3
        ...
        rs.Open("SELECT p.codigo,cd.cantidad,cd.precio_unitario,cd.descripcion,cd.unidad FROM cotizacion_detalle cd " & _
                "JOIN producto p ON p.id_producto=cd.id_producto WHERE cd.id_cotizacion='" & id_cotizacion & "'", conn)
        If rs.RecordCount > 0 Then
            While Not rs.EOF
                ...
                Dim P5 As New PdfPCell(New Paragraph(FormatCurrency(rs.Fields("precio_unitario").Value), FontFactory.GetFont("Arial", 8, Font.NORMAL)))
                ...
                Dim P6 As New PdfPCell(New Paragraph(FormatCurrency(rs.Fields("precio_unitario").Value * rs.Fields("cantidad").Value), FontFactory.GetFont("Arial", 8, Font.NORMAL)))
        ...
        Dim S2 As New PdfPCell(New Paragraph("", FontFactory.GetFont("Arial", 8, Font.UNDERLINE)))
        ...
        Dim S3 As New PdfPCell(New Paragraph("", FontFactory.GetFont("Arial", 8, Font.BOLDITALIC))) ' enganche
        ...
        Dim S4 As New PdfPCell(New Paragraph("", FontFactory.GetFont("Arial", 8, Font.UNDERLINE)))
        ...
        Dim T1 As New PdfPCell(New Paragraph("", FontFactory.GetFont("Arial", 8, Font.BOLDITALIC)))
        T1.Colspan = 2
        T1.MinimumHeight = 20
        ...
        Dim T2 As New PdfPCell(New Paragraph("", FontFactory.GetFont("Arial", 8, Font.UNDERLINE)))
        ...
        Dim T3 As New PdfPCell(New Paragraph("", FontFactory.GetFont("Arial", 8, Font.BOLDITALIC)))
        ...
        Dim V2 As New PdfPCell(New Paragraph(Letras(total_cotizacion), FontFactory.GetFont("Arial", 8, Font.BOLD)))
        ...
        Dim X7 As New PdfPCell(New Paragraph(FormatCurrency(total_cotizacion), FontFactory.GetFont("Arial", 12, Font.BOLD)))
        X7.BorderWidth = 1
        X7.Colspan = 2
        X7.MinimumHeight = 40
        X7.PaddingTop = 15

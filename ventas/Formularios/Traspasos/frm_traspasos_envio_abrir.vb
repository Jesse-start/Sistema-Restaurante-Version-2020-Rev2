Imports System.Math
Public Class frm_traspasos_envio_abrir
    Dim i As Integer
    ...
    Private Sub frm_compras_abrir_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        With lista_traspasos
            ...
            .Columns.Add("Destino", 150, HorizontalAlignment.Right)
    ...
    Private Sub cargar(Optional ByVal pagina As Integer = 1)
        ...
        If Trim(tb_numero.Text) <> "" Then
            Try
                filtro_sql = filtro_sql & "AND id_traspaso_env like '%" & CType(tb_numero.Text, Integer) & "%'"
        ...
        If cb_sucursales.SelectedIndex > -1 Then
            If CType(cb_sucursales.SelectedItem, MyItem).Value > 0 Then
                filtro_sql = filtro_sql & " AND id_sucursal_destino= " & CType(cb_sucursales.SelectedItem, myItem).Value
            End If
        End If

        rs.Open("select distinct year(fecha) anio from traspaso_env WHERE 1 " & filtro_sql & " ORDER by fecha DESC", conn)
        While Not rs.EOF
        ...
        rs.Open("SELECT count(*) as total_traspasos FROM traspaso_env WHERE 1 " & filtro_sql, conn)
        ...
        rs.Open("SELECT id_traspaso_env,sucursal_destino,fecha,YEAR(fecha) AS anio FROM traspaso_env WHERE 1 " & filtro_sql & " ORDER BY id_traspaso_env DESC LIMIT " & inicial & "," & reg_por_pag, conn)
        i = 0
        While Not rs.EOF
            Dim str(2) As String

            str(0) = rs.Fields("id_traspaso_env").Value
            str(1) = rs.Fields("sucursal_destino").Value
            ...
            lista_traspasos.Items(i).Tag = rs.Fields("id_traspaso_env").Value
            rs.MoveNext()
            i = i + 1
        End While
    ...
    Private Sub btn_aceptar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_aceptar.Click
        If lista_traspasos.SelectedItems.Count > 0 Then
            frm_traspasos_envio.cargar(lista_traspasos.SelectedItems.Item(0).Tag)
            Me.Close()
        End If
    End Sub

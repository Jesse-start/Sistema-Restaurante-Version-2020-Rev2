...
Module Nota_Credito
    ...
    Private Sub obtener_valores_xml(ByVal id_nota_credito As Integer)
    ...
    Public Sub Generar_nota_credito(ByVal id_nota_credito As Integer, ByVal concepto_general As Boolean, ByVal cadena_concepto As String)
        ...
        'Conectar()
        rs.Open("SELECT serie FROM nota_credito WHERE id_nota_credito=" & id_nota_credito, conn)
        If rs.RecordCount > 0 Then
            num_serie = rs.Fields("serie").Value
        End If
        rs.Close()
        'conn.close()
        obtener_valores_xml(id_nota_credito)

        Dim filename As String = Application.StartupPath & "/CFDI3.3/xml/" & global_rfc & num_serie & Format(id_nota_credito, "0000000000") & ".pdf"
        Dim file As New FileStream(filename, FileMode.Create, FileAccess.Write, FileShare.ReadWrite)
        Dim writer As PdfWriter = PdfWriter.GetInstance(doc, file)
        doc.Open()
        ExportarDatosPDF_SerieA(doc, writer, id_nota_credito, concepto_general, cadena_concepto)
        doc.Close()
    ...
    Public Function GetColumnasSize(ByVal dg As DataGridView) As Single()
        ...
        Return values
    End Function
    Public Sub ExportarDatosPDF_SerieA(ByVal document As Document, ByVal writer As PdfWriter, ByVal id_nota_credito As Integer, ByVal concepto_general As Boolean, ByVal cadena_concepto As String)
        '=========================================================================
        ...
        'Conectar()
        rs.Open("SELECT nota_credito.fecha,nota_credito.subtotal,nota_credito.iva,nota_credito.total,nota_credito.serie,nota_credito.condiciones_pago,CONCAT(ctlg_uso_cfdi.clave,' ',ctlg_uso_cfdi.nombre) AS uso_cfdi,pago_cuenta,cliente.id_cliente,cliente.id_domicilio, " & _
                ...
                "(SELECT CASE WHEN cliente.id_persona = 0 THEN  empresa.rfc ELSE persona.rfc END as rfc FROM cliente JOIN  nota_credito ON nota_credito.id_cliente=cliente.id_cliente LEFT OUTER JOIN persona ON persona.id_persona = cliente.id_persona LEFT OUTER JOIN empresa ON cliente.id_empresa = empresa.id_empresa WHERE nota_credito.id_nota_credito=" & id_nota_credito & ")As RFC " & _
                "FROM nota_credito " & _
                "JOIN cliente ON nota_credito.id_cliente=cliente.id_cliente " & _
                "JOIN ctlg_uso_cfdi ON ctlg_uso_cfdi.id_uso_cfdi=nota_credito.id_uso_cfdi " & _
        ...
        rw.Open("SELECT CONCAT(fp.clave,' ',fp.nombre) AS forma_pago,CONCAT(mp.clave,' ',mp.nombre) AS metodo_pago " &
                   ...
                   "JOIN ctlg_forma_pago fp ON fp.id_forma_pago=nc.id_forma_pago " &
        ...
        Dim factura As New PdfPCell(New Paragraph("NOTA DE CRÉDITO", ArialMT13))
        ...
        Dim email As New PdfPCell(New Paragraph("Tipo de Comprobante: E Egreso   Atención a clientes:" & emisor_correo & " Tel.: " & emisor_telefono, ArialMT7))
        email.Colspan = 7
        ...
        Else
            'Conectar()
            rs.Open("SELECT pc.clave AS producto_clave,p.nombre,ncd.cantidad,ic.codigo AS clave_impuesto,i.nombre AS nombre_impuesto,ncd.total_impuesto,u.nombre AS unidad, u.clave AS clave_unidad,ncd.tasa_impuesto,ncd.precio,ncd.importe,ncd.descuento " & _
            ...
            If rs.RecordCount > 0 Then
                ...
                While Not rs.EOF
                    ...
                    Dim concepto001 As New PdfPCell(New Paragraph(rs.Fields("nombre").Value & vbCrLf & "Descuento:" & FormatCurrency(rs.Fields("descuento").Value) & " Impuestos: Traslado " & rs.Fields("clave_impuesto").Value & " " & rs.Fields("nombre_impuesto").Value & ":" & FormatCurrency(rs.Fields("total_impuesto").Value) & " Factor:Tasa", Futuran8))
                    concepto001.MinimumHeight = 15
        ...
        codigo_seriea = iTextSharp.text.Image.GetInstance(Application.StartupPath & "/CFDI3.3/xml/" & global_rfc & num_serie & Format(id_nota_credito, "0000000000") & ".png")

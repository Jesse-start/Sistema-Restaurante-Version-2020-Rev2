Imports System.Math
Public Class frm_facturacion_electronica_abrir
    Dim i As Integer
    ...
    Private Sub cargar(Optional ByVal pagina As Integer = 1)
        ...
        rs.Open("select distinct year(fecha) anio from factura_electronica WHERE 1 " & filtro_sql & " ORDER by fecha DESC", conn)
        While Not rs.EOF
        ...
        rs.Open("SELECT count(*) as total_facturas FROM factura_electronica WHERE 1 " & filtro_sql, conn)
        ...
        rs.Open("SELECT id_factura_electronica,numero,total, YEAR(fecha) anio,fecha FROM factura_electronica WHERE 1 " & filtro_sql & " ORDER BY id_factura_electronica DESC LIMIT " & inicial & "," & reg_por_pag, conn)
        i = 0
        While Not rs.EOF
            Dim str(3) As String

            str(0) = rs.Fields("numero").Value
            ...
            lista_factura.Items(i).Tag = rs.Fields("id_factura_electronica").Value
            rs.MoveNext()
            i = i + 1
        End While
        ...
        End If
        TextBox1.Text = "SELECT id_factura_electronica, YEAR(fecha) anio,fecha FROM factura_electronica WHERE 1 " & filtro_sql & " ORDER BY id_factura_electronica DESC LIMIT " & inicial & "," & reg_por_pag
    End Sub
    ...
    Private Sub btn_aceptar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_aceptar.Click
        If lista_factura.SelectedItems.Count > 0 Then
            If global_frm_factura = 1 Then
                frm_facturacion_electronica.cargar_factura(lista_factura.SelectedItems.Item(0).Tag)
            Else

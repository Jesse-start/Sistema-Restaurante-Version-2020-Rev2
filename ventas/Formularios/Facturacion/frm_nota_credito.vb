...
Public Class frm_compras
    ...
    Public Sub limpiar_nota_credito()
        ...
        Next
    End Sub
    Public Sub limpiar_nota_credito_productos()
        For x = 0 To 20
            For j = 0 To 7
    ...
    End Sub
    Private Sub configuracion_tabla_nota_credito()
        tabla = New DataTable("nota_credito")
    ...
    Private Sub frm_factura_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        configuracion_tabla_nota_credito()
    ...
    Private Sub configuracion_listas()
        ...
        For i = 0 To lst_facturas.Items.Count - 2 Step 2
    ...
    End Sub
    Public Sub cargar_nota_credito(Optional ByVal id As Integer = 0)

        id_nota_credito = id

        xmlFile = Application.StartupPath & "/CFDI3.3/temp/" & global_rfc & num_serie & Format(id_nota_credito, "0000000000") & ".xml"
        xmlTimbradoFile = Application.StartupPath & "/CFDI3.3/XML/" & global_rfc & num_serie & Format(id_nota_credito, "0000000000") & ".xml"
        ...
        If id_nota_credito > 0 Then
            ...
            rs.Open("SELECT id_nota_credito,total,subtotal,iva,id_cliente,id_empleado,fecha,year(fecha) anio,id_forma_pago,id_metodo_pago,condiciones_pago,pago_cuenta,estatus_timbrado,estatus " & _
                    "FROM nota_credito " & _
            ...
            tb_numero_factura.Text = num_serie & id_nota_credito
            ...
            rs2.Open("SELECT ncd.*, nc.id_cliente,p.nombre as producto,u.nombre as unidad " & _
                     "FROM nota_credito_detalle ncd " & _
                     "JOIN nota_credito nc ON nc.id_nota_credito=ncd.id_nota_credito " & _
            ...
            If rs2.RecordCount > 0 Then
                While Not rs2.EOF
                    cargar_producto_nota_credito(rs2.Fields("producto").Value, rs2.Fields("cantidad").Value, rs2.Fields("unidad").Value, rs2.Fields("nombre_impuesto").Value, rs2.Fields("precio").Value, rs2.Fields("importe").Value, rs2.Fields("total_impuesto").Value, rs2.Fields("tasa_impuesto").Value, rs2.Fields("id_producto").Value)
                    rs2.MoveNext()
                End While
            End If
            ...
            rs.Open("SELECT ncd.id_tipo_relacion, CONCAT(tr.clave,' ',tr.nombre) AS tipo_relacion,ncd.UUID " & _
                    "FROM nota_credito_docs ncd " & _
                    "JOIN ctlg_tipo_relacion tr ON tr.id_tipo_relacion=ncd.id_tipo_relacion " & _
                    "WHERE ncd.id_nota_credito=" & id_nota_credito, conn)
            If rs.RecordCount > 0 Then
                i = 0
                While Not rs.EOF
        ...
        Else
            ...
            rs.Open("SELECT IF(MAX(id_nota_credito) IS NULL,1,MAX(id_nota_credito)+1) numero from nota_credito WHERE YEAR(fecha) = '" & Date.Today.Year & "'", conn)
    ...
    Private Sub agregar_producto(ByVal descripcion As String, ByVal cantidad As Decimal, ByVal unidad As String, ByVal impuesto As String, ByVal precio As Decimal, ByVal importe As Decimal, ByVal total_impuesto As Decimal, ByVal tasa_impuesto As Decimal, Optional ByVal id_producto As Integer = 0)
...
NEW_LINE:   total_productos = total_productos + 1
        ...
        End If
    End Sub
    Private Sub cargar_producto_nota_credito(ByVal descripcion As String, ByVal cantidad As Decimal, ByVal unidad As String, ByVal impuesto As String, ByVal precio As Decimal, ByVal importe As Decimal, ByVal total_impuesto As Decimal, ByVal tasa_impuesto As Decimal, Optional ByVal id_producto As Integer = 0)
    ...
    Public Sub agregar_producto(ByVal id As Integer, Optional ByVal actualizar As Boolean = False, Optional ByVal cantidad As Decimal = 0)
        ...
        Else
            ...
            tabla_linea("precio") = rs.Fields("precio").Value / (1 + rs.Fields("tasa_impuesto").Value)
    ...
    Private Sub actualizar_totales()
        ...
        tb_total.Text = "$ " & Math.Round(subtotal + total_impuestos, 2)
    End Sub
    ...
    Private Sub tsb_imprimir_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        imprimir_recibo_prefactura(id_nota_credito)
    ...
    Private Sub preparar_documento(ByVal tipo_documento As Integer, ByVal id_documento As Integer, ByVal direccion As String, ByVal ciudad As String, ByVal cp As String)
        ...
        If tipo_documento = 1 Then '-----1.- factura
            ...
            Dim i As Integer = 0
            limpiar_nota_credito_productos()
    ...
    Private Sub btn_cancelar_factura_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_cancelar.Click
        If MsgBox("Esta seguro que desea cancelar esta nota de crédito", MsgBoxStyle.YesNo + MsgBoxStyle.Information) = MsgBoxResult.Yes Then

            ' conn.Execute("UPDATE venta SET id_factura='0' WHERE id_factura=" & id_nota_credito)
            conn.Execute("UPDATE nota_credito SET estatus='CANCELADA' WHERE id_nota_credito=" & id_nota_credito)

            cancelar_finkok_nota()

            cargar_nota_credito(id_nota_credito)
            MsgBox("Nota Credito cancelada correctamente", MsgBoxStyle.Information, "Aviso")
        End If
    End Sub
    ...
    End Function
    Private Sub cancelar_finkok_nota()

        Dim xmlTimbradoFile As String = Application.StartupPath & "/CFDI3.3/xml/" & global_rfc & num_serie & Format(id_nota_credito, "0000000000") & ".xml"
                    ...
                    If reader.HasAttributes Then 'If attributes exist
                        While reader.MoveToNextAttribute()
                            ...
                            If reader.Name = "UUID" Then
                                uuid_nota_credito_timbra = reader.Value
                            End If
                        End While
                    End If
            End Select
        ...
        Try
            FabricaPEM()
            Dim uuid() As String
            uuid = {uuid_nota_credito_timbra}
            Dim folio_fiscal_cancelar As String = uuid_nota_credito_timbra
            Dim rfc As String
    ...
    End Sub
    Private Sub timbrar_finkok_nota()
        ...
        Try
            ...
            conn.Execute("UPDATE nota_credito SET estatus_timbrado='TIMBRADA' WHERE id_nota_credito=" & id_nota_credito)
    ...
    End Sub
    Private Sub generar_formato_pdf_nota()
        Dim xmlTimbradoFile As String = Application.StartupPath & "/CFDI3.3/xml/" & global_rfc & num_serie & Format(id_nota_credito, "0000000000") & ".xml"
        Dim reader As XmlTextReader = New XmlTextReader(xmlTimbradoFile)
        Do While (reader.Read())
            Select Case reader.NodeType
        ...
        QrCodeImgControl1.Text = "https://verificacfdi.facturaelectronica.sat.gob.mx/default.aspx?id=" & uuid_nota_credito_timbra & "&re=" & global_rfc & "&rr=" & rfc_receptor & "&tt=" & total_factura & "&fe=" & sello_digital_emisor.Substring(sello_digital_emisor.Length - 8, 8)
        Dim qrcode As System.Drawing.Image = DirectCast(QrCodeImgControl1.Image.Clone, Image)
        qrcode.Save(Application.StartupPath & "/CFDI3.3/xml/" & global_rfc & num_serie & Format(id_nota_credito, "0000000000") & ".png")
        qrcode.Dispose()

        Generar_nota_credito(id_nota_credito, False, "")
    End Sub
    ...
    Private Sub btn_guardar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_guardar.Click
        ...
        If correcto = True Then
            If id_nota_credito > 0 Then
                ...
                conn.Execute("UPDATE nota_credito SET serie='" & num_serie & "',fecha=NOW(),subtotal='" & CDec(tb_subtotal.Text) & "',iva='" & CDec(tb_impuestos.Text) & "',total='" & CDec(tb_total.Text) & "',id_cliente=" & id_cliente & ",descuento='" & cliente_descuento & "',id_empleado=" & id_empleado & ",id_forma_pago='" & CType(cb_pago_forma.SelectedItem, myItem).Value & "',id_metodo_pago='" & CType(cb_pago_metodo.SelectedItem, myItem).Value & "',id_uso_cfdi='" & CType(cb_uso_cfdi.SelectedItem, myItem).Value & "',pago_cuenta='" & tb_pago_cuenta.Text & "',condiciones_pago='" & tb_pago_condiciones.Text & "' WHERE id_nota_credito='" & id_nota_credito & "'")
                conn.Execute("DELETE FROM nota_credito_detalle WHERE id_nota_credito='" & id_nota_credito & "'")

                For row = 0 To tabla.Rows.Count - 1
                    conn.Execute("INSERT INTO nota_credito_detalle (id_nota_credito,id_producto,cantidad,total_impuesto,tasa_impuesto,nombre_impuesto,precio,importe) VALUES" & _
                                 " (" & id_nota_credito & "," & dg_factura.Item("id_producto", row).Value & ",'" & dg_factura.Item("cantidad", row).Value & "'," & dg_factura.Item("total_impuesto", row).Value & ",'" & dg_factura.Item("tasa_impuesto", row).Value & "','" & dg_factura.Item("impuesto", row).Value & "','" & dg_factura.Item("precio", row).Value & "','" & dg_factura.Item("importe", row).Value & "')")

                Next

                conn.Execute("DELETE FROM nota_credito_docs WHERE id_nota_credito='" & id_nota_credito & "'")
                For x = 0 To lst_documentos.Items.Count - 1
                    conn.Execute("INSERT INTO nota_credito_docs(id_nota_credito,id_tipo_relacion,UUID) VALUES('" & id_nota_credito & "','" & lst_documentos.Items(x).Tag & "','" & lst_documentos.Items(x).SubItems(1).Text & "')")
                Next
                ...
                MsgBox("La nota de crédito " & tb_numero_factura.Text & "se ha actualizado!" & vbCrLf, MsgBoxStyle.Information, "Aviso")
            ...
            Else
                ...
                conn.Execute("INSERT INTO nota_credito (serie,fecha,subtotal,iva,total,id_cliente,descuento,id_empleado,id_forma_pago,id_metodo_pago,id_uso_cfdi,pago_cuenta,condiciones_pago) VALUES" & _
                             " ('" & num_serie & "',NOW(),'" & CDec(tb_subtotal.Text) & "','" & CDec(tb_impuestos.Text) & "','" & CDec(tb_total.Text) & "'," & id_cliente & ",'" & cliente_descuento & "'," & id_empleado & ",'" & CType(cb_pago_forma.SelectedItem, myItem).Value & "','" & CType(cb_pago_metodo.SelectedItem, myItem).Value & "','" & CType(cb_uso_cfdi.SelectedItem, myItem).Value & "','" & tb_pago_cuenta.Text & "','" & tb_pago_condiciones.Text & "')")

                rs.Open("SELECT last_insert_id() id_nota_credito", conn)
                id_nota_credito = rs.Fields("id_nota_credito").Value
                rs.Close()


                For row = 0 To tabla.Rows.Count - 1
                    conn.Execute("INSERT INTO nota_credito_detalle (id_nota_credito,id_producto,cantidad,total_impuesto,tasa_impuesto,nombre_impuesto,precio,importe) VALUES" & _
                                 " (" & id_nota_credito & "," & dg_factura.Item("id_producto", row).Value & "," & dg_factura.Item("cantidad", row).Value & ",'" & dg_factura.Item("total_impuesto", row).Value & "','" & dg_factura.Item("tasa_impuesto", row).Value & "','" & dg_factura.Item("impuesto", row).Value & "','" & CDec(dg_factura.Item("precio", row).Value) & "','" & CDec(dg_factura.Item("importe", row).Value) & "')")
                ...
                btn_guardar.Enabled = False
                cargar_nota_credito(id_nota_credito)
                MsgBox("La Nota de Crédito se ha generado con el Número " & vbCrLf & tb_numero_factura.Text, MsgBoxStyle.Information, "Aviso")
            End If
            cargar_nota_credito_cliente(CType(cb_cliente_busqueda.SelectedItem, myItem).Value, tb_buscar.Text)
        Else
            MsgBox(mensaje)
        End If
    End Sub
    ...
    Private Sub btn_timbrar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_timbrar.Click
        Try
            ...
            Genera_CFDI3_3_nota(id_nota_credito, num_serie)
            timbrar_finkok_nota()
            generar_formato_pdf_nota()
            cargar_nota_credito(id_nota_credito)
        Catch ex As Exception
    ...
    Private Sub btn_enviar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_enviar.Click
        enviar_nota_credito_email(id_nota_credito)
    End Sub
        ...
        Try
            ...
            With FolderBrowserDialog
                ...
                If ret = Windows.Forms.DialogResult.OK Then
                    ...
                    Dim Source As String = Application.StartupPath & "/CFDI3.3/XML/" & global_rfc & num_serie & Format(id_nota_credito, "0000000000") & ".xml"
                    Dim Destination As String = ruta_destino_xml & "/" & global_rfc & num_serie & Format(id_nota_credito, "0000000000") & ".xml"
    ...
    Private Sub cb_cliente_busqueda_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cb_cliente_busqueda.SelectedIndexChanged
        cargar_nota_credito_cliente(CType(cb_cliente_busqueda.SelectedItem, myItem).Value, tb_buscar.Text)
    End Sub
    Private Sub cargar_nota_credito_cliente(ByVal id_cliente As Integer, ByVal buscar As String)
        Dim filtro As String = ""
        Dim i As Integer = 0

        lst_facturas.Items.Clear()
        If id_cliente > 0 Then
            filtro = " WHERE n.id_cliente='" & id_cliente & "'"
        End If

        If buscar.Length > 0 Then
            If id_cliente > 0 Then
                filtro = filtro & " AND n.id_nota_credito LIKE '%" & buscar & "%'"
            Else
                filtro = " WHERE n.id_nota_credito LIKE '%" & buscar & "%'"
            End If

        End If

        rs.Open("SELECT n.id_nota_credito,n.serie,n.fecha,n.total, CASE WHEN c.id_persona = 0 THEN  e.razon_social ELSE CONCAT(p.nombre,' ',p.ap_paterno,' ',p.ap_materno) END AS razon_social, CASE WHEN c.id_persona = 0 THEN  e.rfc ELSE p.rfc END AS registro " & _
                "FROM nota_credito n " & _
                "JOIN cliente c ON c.id_cliente=n.id_cliente " & _
                "LEFT OUTER JOIN persona p ON c.id_persona = p.id_persona " & _
                "LEFT OUTER JOIN empresa e ON c.id_empresa = e.id_empresa " & filtro, conn)
        If rs.RecordCount > 0 Then

            While Not rs.EOF
                Dim str(3) As String
                str(0) = rs.Fields("serie").Value & rs.Fields("id_nota_credito").Value
                ...
                lst_facturas.Items(i).Tag = rs.Fields("id_nota_credito").Value
                rs.MoveNext()
                i = i + 1
    ...
    Private Sub lst_facturas_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles lst_facturas.DoubleClick
        If lst_facturas.SelectedItems.Count > 0 Then
            cargar_nota_credito(lst_facturas.SelectedItems.Item(0).Tag)
        End If
    End Sub
